...
async function destroyResources(yamlTemplate) {
    ...
    const destroyResource = (properties) => dynamo.deleteTable({ TableName: properties.TableName }).promise();
...
switch (process.env.ACTION) {
    case 'CREATE': action = createResources(cloudFormation); break;
    case 'DELETE': action = destroyResources(cloudFormation); break;
